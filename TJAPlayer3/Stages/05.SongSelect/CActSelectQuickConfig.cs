	...
	internal class CActSelectQuickConfig : CActSelectPopupMenu
	{
		...
		private void CActSelectQuickConfigMain()
		{
			...
			*/
			...
			for (int nConfSet = 0; nConfSet < Enum.GetValues(typeof(E楽器パート)).Length - 1; nConfSet++)
			{
				lci.Add(new List<List<CItemBase>>());                                   // ConfSet用の3つ分の枠。
				for (int nInst = 0; nInst < Enum.GetValues(typeof(E楽器パート)).Length - 1; nInst++)
				{
		...
		public override void tEnter押下Main(int nSortOrder)
		{
			switch (n現在の選択行)
			{
				...
				case (int)EOrder.GameMode:
					...
					switch (GetIndex((int)EOrder.GameMode))
					{
		...
		#region [ private ]
		//-----------------
		private int nCurrentTarget = (int)E楽器パート.TAIKO;
