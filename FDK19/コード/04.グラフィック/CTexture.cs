...
namespace FDK
{
	public class CTexture : IDisposable
	{
		...
		public void t2D左右反転描画(Device device, float x, float y, float depth, Rectangle rc画像内の描画領域)
		{
			...
			float w = rc画像内の描画領域.Width * this.vc拡大縮小倍率.X * CTexture.f画面比率;
			float h = rc画像内の描画領域.Height * this.vc拡大縮小倍率.Y * CTexture.f画面比率;
			float f左U値 = rc画像内の描画領域.Left / ((float)this.szテクスチャサイズ.Width);
			float f右U値 = rc画像内の描画領域.Right / ((float)this.szテクスチャサイズ.Width);
			float f上V値 = rc画像内の描画領域.Top / ((float)this.szテクスチャサイズ.Height);
			float f下V値 = rc画像内の描画領域.Bottom / ((float)this.szテクスチャサイズ.Height);
			this.color4.Alpha = _opacity / 255f;
			int color = this.color4.ToArgb();
